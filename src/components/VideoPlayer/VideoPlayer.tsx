...
export default function VideoPlayer({
  ...
  const episodeCompletedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    progressSaveTimer.current = setInterval(() => {
      ...
      fetch(
        ...
        .then((data) => {
          // Если эпизод только что завершён — вызываем коллбэк
          if (data?.completed && !episodeCompletedRef.current) {
  ...
  const setMarkerAtCurrentTime = (field: keyof Markers) => {
    ...
    setEditMarkers((prev) => ({
      ...prev,
      [field]: Math.round(v.currentTime * 100) / 100,
    }));
  };
  ...
  const fetchAndResume = useCallback(
    async (video: HTMLVideoElement) => {
      if (!userId || !currentEpisodeId) return;
      try {
        const res = await fetch(
          `${API_URL}/api/watch-progress?userId=${userId}&episodeId=${currentEpisodeId}`,
        );
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      if (
        ...
        markers?.outroStart != null &&
        markers?.outroEnd != null
      ) {
    ...
    const onEnded = () => {
      setPlaying(false);
      if (autoplayNextRef.current && nextEp && onEpisodeChange)
  ...
  const handleProgressTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (bar && v && duration && e.changedTouches[0]) {
      ...
      const pct = Math.max(
        0,
        Math.min(1, (e.changedTouches[0].clientX - rect.left) / rect.width),
      );
      v.currentTime = pct * duration;
    }
    setSeeking(false);
  ...
  return (
    ...
    >
      <video
        ...
        onTouchEnd={(e) => {
          ...
          const side: "left" | "right" =
            tapX < rect.left + rect.width / 2 ? "left" : "right";
          const now = Date.now();
          if (now - lastTapTime.current < 300 && lastTapSide.current === side) {
            ...
            if (v) {
              ...
              v.currentTime = Math.max(
                0,
                Math.min(v.duration, v.currentTime + seekAmount),
              );
      ...
      {doubleTapSide && (
        <div
          className={`${styles.doubleTapOverlay} ${doubleTapSide === "left" ? styles.doubleTapLeft : styles.doubleTapRight}`}
        >
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.controlsRow}>
                ...
                {showVolume && (
                  <div className={styles.volumeSlider}>
                    <input
                      ...
                      style={{
                        ["--vol-pct" as string]: `${(muted ? 0 : volume) * 100}%`,
                      }}
                    />
                  </div>
                )}
          ...
          <div className={styles.controlsRight}>
                ...
                <>
                  ...
                  <div className={styles.settingsPanel}>
                    <div className={styles.settingsTabs}>
                      {(
                        [
                          ...
                          {
                            id: "speed" as SettingsTab,
                            label: "Скорость",
                            icon: <Gauge size={14} />,
                          },
                          {
                            id: "autoplay" as SettingsTab,
                            label: "Авто",
                            icon: <FastForward size={14} />,
                          },
                          {
                            id: "skip" as SettingsTab,
                            label: "Опенинг/Эндинг",
                            icon: <SkipIntro size={14} />,
                          },
                          ...(!isMobile
                            ? [
                                {
                    ...
                    <div className={styles.settingsBody}>
                      {/* Quality */}
                      <div
                        className={`${styles.settingsPane} ${settingsTab === "quality" ? styles.settingsPaneActive : ""}`}
                      >
                        <button
                          ...
                          {currentQuality === -1 &&
                            ...
                            hlsRef.current.currentLevel >= 0 && (
                              <span className={styles.settingsBadge}>
                                {qualities[hlsRef.current.currentLevel]?.label}
                              </span>
                            )}
                        </button>
                        {[...qualities]
                      ...
                      {/* Speed */}
                      <div
                        className={`${styles.settingsPane} ${settingsTab === "speed" ? styles.settingsPaneActive : ""}`}
                      >
                        {SPEED_OPTIONS.map((s) => (
                          <button
                            key={s}
                      ...
                      {/* Autoplay */}
                      <div
                        className={`${styles.settingsPane} ${settingsTab === "autoplay" ? styles.settingsPaneActive : ""}`}
                      >
                        <button
                        ...
                        >
                          ...
                          <span
                            className={`${styles.toggle} ${autoplayNext ? styles.toggleOn : styles.toggleOff}`}
                          />
                        </button>
                      </div>
                      ...
                      {/* Skip Intro/Outro */}
                      <div
                        className={`${styles.settingsPane} ${settingsTab === "skip" ? styles.settingsPaneActive : ""}`}
                      >
                        <button
                          className={styles.settingsToggleRow}
                        ...
                        >
                          ...
                          <span
                            className={`${styles.toggle} ${skipIntro ? styles.toggleOn : styles.toggleOff}`}
                          />
                        </button>
                        {canEditMarkers &&
                          markers?.introStart != null &&
                          markers?.introEnd != null && (
                            <div className={styles.markerInfo}>
                              {formatTime(markers.introStart)} →{" "}
                              {formatTime(markers.introEnd)}
                            </div>
                          )}
                        <button
                          className={styles.settingsToggleRow}
                        ...
                        >
                          ...
                          <span
                            className={`${styles.toggle} ${skipOutro ? styles.toggleOn : styles.toggleOff}`}
                          />
                        </button>
                        {canEditMarkers &&
                          markers?.outroStart != null &&
                          markers?.outroEnd != null && (
                            <div className={styles.markerInfo}>
                              {formatTime(markers.outroStart)} →{" "}
                              {formatTime(markers.outroEnd)}
                            </div>
                          )}

                        {canEditMarkers && (
                          <div className={styles.markerEditor}>
                            <div className={styles.markerEditorTitle}>
                              Редактор меток
                            </div>
                            <div className={styles.markerGroup}>
                              ...
                              <div className={styles.markerRow}>
                                <button
                                  ...
                                  onClick={() =>
                                    setMarkerAtCurrentTime("introStart")
                                  }
                                  title="Начало опенинга = текущее время"
                                >
                                  Начало:{" "}
                                  {editMarkers.introStart != null
                                    ? formatTime(editMarkers.introStart)
                                    : "—"}
                                </button>
                                {editMarkers.introStart != null && (
                                  <button
                                    className={styles.markerClear}
                                    onClick={() => clearMarker("introStart")}
                                  >
                                    ✕
                                  </button>
                                )}
                              ...
                              <div className={styles.markerRow}>
                                <button
                                  className={styles.markerBtn}
                                  onClick={() =>
                                    setMarkerAtCurrentTime("introEnd")
                                  }
                                  title="Конец опенинга = текущее время"
                                >
                                  Конец:{" "}
                                  {editMarkers.introEnd != null
                                    ? formatTime(editMarkers.introEnd)
                                    : "—"}
                                </button>
                                {editMarkers.introEnd != null && (
                                  <button
                                    className={styles.markerClear}
                                    onClick={() => clearMarker("introEnd")}
                                  >
                                    ✕
                                  </button>
                                )}
                            ...
                            <div className={styles.markerGroup}>
                              ...
                              <div className={styles.markerRow}>
                                <button
                                  className={styles.markerBtn}
                                  onClick={() =>
                                    setMarkerAtCurrentTime("outroStart")
                                  }
                                  title="Начало эндинга = текущее время"
                                >
                                  Начало:{" "}
                                  {editMarkers.outroStart != null
                                    ? formatTime(editMarkers.outroStart)
                                    : "—"}
                                </button>
                                {editMarkers.outroStart != null && (
                                  <button
                                    className={styles.markerClear}
                                    onClick={() => clearMarker("outroStart")}
                                  >
                                    ✕
                                  </button>
                                )}
                              ...
                              <div className={styles.markerRow}>
                                <button
                                  className={styles.markerBtn}
                                  onClick={() =>
                                    setMarkerAtCurrentTime("outroEnd")
                                  }
                                  title="Конец эндинга = текущее время"
                                >
                                  Конец:{" "}
                                  {editMarkers.outroEnd != null
                                    ? formatTime(editMarkers.outroEnd)
                                    : "—"}
                                </button>
                                {editMarkers.outroEnd != null && (
                                  <button
                                    className={styles.markerClear}
                                    onClick={() => clearMarker("outroEnd")}
                                  >
                                    ✕
                                  </button>
                                )}
                      ...
                      {/* Shortcuts */}
                      <div
                        className={`${styles.settingsPane} ${settingsTab === "shortcuts" ? styles.settingsPaneActive : ""}`}
                      >
                        {SHORTCUTS.map((sc, i) => (
                        ...
                        <div className={styles.shortcutNote}>
                          Работает и на русской раскладке
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {!isMobile && (
              <button
                ...
                title={
                  isFullscreen ? "Выход из полноэкранного" : "Полный экран"
                }
              >
          ...
          {isMobile && !showSettings && !showEpisodes && (
            <button
              className={styles.mobileFullscreen}
              onClick={toggleFullscreen}
              title={isFullscreen ? "Выход из полноэкранного" : "Полный экран"}
            >
              {isFullscreen ? <Minimize size={18} /> : <Maximize size={18} />}
            </button>
          )}
