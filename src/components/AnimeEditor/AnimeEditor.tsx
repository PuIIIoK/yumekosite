...
import styles from "@/app/admin/create-anime/createAnime.module.scss";
...
export default function AnimeEditor() {
  ...
  // Role model:
  //  - priority >= 80  → admin/moderator: changes apply instantly
  //  - everyone else    → can only edit existing titles, changes go to moderation
  const rolesList = auth.user?.roles ?? [auth.user?.role].filter(Boolean);
  const maxPri = Math.max(0, ...rolesList.map((r) => r?.priority ?? 0));
  const isModerator = maxPri >= 80;

  // Where the "back"/"cancel" buttons return to: the release page when editing,
  // otherwise the admin dashboard (create flow).
  const exitHref = isEdit ? `/realeses/anime-page/${editId}` : "/admin";

  useEffect(() => {
    // Regular users may only reach this page in edit mode (their changes go to
    // moderation); moderators/admins can always create and edit directly.
    const allowed = auth.isAuthenticated && (isModerator || isEdit);
    if (!allowed) {
      router.replace("/");
      return;
    }
      ...
      fetch(`${API_URL}/api/anime/${editId}`)
        ...
        .then((data) => {
          setForm({
            ...
            hiddenStudio: data.hiddenStudio || "",
          });
  ...
  }, [auth, router, editId, isEdit, isModerator]);

  const setField = (key: keyof AnimeForm, value: string | boolean) => {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // Regular users editing an existing title → send the changes to
      // moderation instead of applying them directly.
      if (isEdit && !isModerator) {
        const res = await fetch(`${API_URL}/api/anime-edit-requests`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!res.ok) {
          ...
          setError(data.error || "Не удалось отправить на модерацию");
          setSaving(false);
          return;
        }
        setSaving(false);
        setSaved(true);
        setTimeout(() => router.push(`/realeses/anime-page/${editId}`), 600);
        return;
      }

      // Admins/moderators (and creation) → apply instantly.
      const url = isEdit
      ...
      setSaved(true);
      setTimeout(() => {
        if (isEdit) router.push(`/realeses/anime-page/${editId}`);
      ...
      <main className={styles.page}>
        ...
        >
          <div className={styles.topBar}>
            <button className={styles.backBtn} onClick={() => router.push(exitHref)}>
            ...
            <div className={styles.topBarRight}>
              <button className={styles.cancelBtn} onClick={() => router.push(exitHref)}>
                Отмена
              </button>
              <button
              ...
              >
                {saved ? (
                  <>
                    <Check size={14} />
                    {isEdit && !isModerator ? "Отправлено" : "Сохранено"}
                  </>
                ) : saving ? (
                  <>
                    <Loader2 size={14} className={styles.saveSpin} />
                    {isEdit && !isModerator ? "Отправляю..." : "Сохраняю..."}
                  </>
                ) : isEdit && !isModerator ? (
                  <>
                    <Send size={14} /> Отправить на модерацию
                  </>
                ) : (
                  <>
            ...
            <div className={styles.fieldsCol}>
              ...
              <section className={styles.section}>
                <h3 className={styles.sectionTitle}>
                  <EyeOff size={16} style={{ marginRight: 6, verticalAlign: -2 }} />
                  Видимость
                </h3>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className={styles.fieldHint}>
                        Тайтл не будет отображаться в каталоге, поиске и на
                        публичной странице релиза.
                      </div>
                    </div>
                  </label>

                  <div className={styles.fieldGroup}>
                    <label className={styles.fieldLabel}>Скрыть студию озвучки</label>
                    <input
                      className={styles.fieldInput}
                      value={form.hiddenStudio}
                      onChange={(e) => setField("hiddenStudio", e.target.value)}
                      placeholder="YumekoStudio, Studio A"
                    />
                    <div className={styles.fieldHint}>
                      Через запятую можно указать несколько студий. Их эпизоды и
                      озвучка будут скрыты на публичных страницах.
                    </div>
                  </div>
                </div>
              </section>
